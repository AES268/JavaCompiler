...
public class Tokenizer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ArrayList<Token> listoftoken = null,listofVariables = null;
        try {
        ...
        Z.checkassignmentexists(listoftoken,listofVariables);Z.checkassignmentorder(listoftoken,listofVariables);
    }
    ...
    ArrayList<Token> isvalidthen(String[] GO) {
        int then_location=0;StringBuilder result = new StringBuilder();
        while(!GO[then_location++].equals("then"));
         for(int i=then_location;i<GO.length-1;i++)
         {result.append(GO[i]+" ");}
         String input=result.toString();
         System.out.println(input);
         Tokenizer thenOutput=new Tokenizer();
         ArrayList<Token> IfResult=null;
         try{IfResult=thenOutput.tokenize(input);}
                ...
                for (int i = 0; i < GO.length; i++) {
                    ...
                    if (k2 == LineType.ASSIGNMENT) {
                        ...
                        if (!isvalidcond(GO)) {throw new Exception("ERROR! INVALID CONDITION STATEMENT");}
                            ...
                            ArrayList<Token>v=isvalidthen(GO);
                            result.add(new Token(token2, nextTokenType(token2), linenum));
                            result.add(new Token("GO", TokenType.GO, linenum));
                        } 
                        else {result.add(new Token(token2, nextTokenType(token2), linenum));}
        ...
        for (String token : tokens2) {
            ...
            try {
                for (int i = 0; i < GO.length; i++) {
                    ...
                    if (k2 == LineType.ASSIGNMENT) {
                        if (isValididentifier(token2)&&nextTokenType(GO[0])!=TokenType.CHAR) {
                            result.add(new Token(token2, nextTokenType(token2), linenum));
                        }
                        else if(nextTokenType(GO[0])==TokenType.CHAR&&!token2.equals(GO[3])&&isValididentifier(token2))
                        {result.add(new Token(token2, nextTokenType(token2), linenum));}
                    } 
                    // else if (k2 == LineType.DECLARATION) {
